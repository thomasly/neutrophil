...
class LossHistory(Callback):
    ...
    def __init__(
        self, epoch_filename, batch_filename, model_file,
    ...
    def on_train_begin(self, logs=None):
        if self.append:
            ...
            self.epoch_csv_file = open(
                self.epoch_filename, 'a' + self.file_flags)
        else:
            self.epoch_csv_file = open(
                self.epoch_filename, 'w' + self.file_flags)
        ...
        if self.append:
            ...
            self.batch_csv_file = open(
                self.batch_filename, 'a' + self.file_flags)
        else:
            self.batch_csv_file = open(
                self.batch_filename, 'w' + self.file_flags)
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if self.model.stop_training:
            ...
            logs = dict(
                [(
                    k, logs[k]
                ) if k in logs else (k, 'NA') for k in self.epoch_keys])
        ...
        row_dict.update(
            (key, handle_value(logs[key])) for key in self.epoch_keys)
    ...
    def on_batch_end(self, batch, logs=None):
        ...
        if self.model.stop_training:
            ...
            logs = dict(
                [(
                    k, logs[k]
                ) if k in logs else (k, 'NA') for k in self.batch_keys])
        ...
        row_dict.update(
            (key, handle_value(logs[key])) for key in self.batch_keys)
